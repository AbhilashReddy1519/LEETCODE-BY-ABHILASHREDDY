...
void slv()
{
  ...
  vc<pi> ans;
  auto work = [&](auto self, vi x, vi y) -> void
  {
    if (si(ans) == n - 1)
      return;
    if (si(x) > si(y))
    ...
    vi p(y.bg, y.bg + si(y) / 2);
    vi q(y.bg + si(y) / 2, y.ed);

    self(self, x, p);
    self(self, x, q);
  };

  rep(i, si(ls)) rng(j, i + 1, si(ls)) work(work, ls[i], ls[j]);

  auto answer = [&](vc<pi> es)
  {
    if (dbg)
    {
      vc<pi> godes;
      rep(i, n) for (auto j : god[i]) if (i < j) godes.eb(i, j);
      soin(godes);
      for (auto &[a, b] : es)
        if (a > b)
          swap(a, b);
